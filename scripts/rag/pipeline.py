"""
RAG (Retrieval-Augmented Generation) pipeline для координации всех компонентов.
"""
...
class RAGPipeline:
    ...
    def index_documents(self, documents: List[str], sources: List[str]) -> bool:
        """
        Индексирует документы в базе знаний.
        ...
        if not self.embedder or not self.vectorstore:
            raise ValueError("Embedder и VectorStore должны быть инициализированы")
